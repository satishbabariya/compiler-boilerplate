...
#ifndef MYLANG_COMMON_INIT_LLVM_H_
#define MYLANG_COMMON_INIT_LLVM_H_
...
// this class should be created in the `main` function of each MyLang binary
// that interacts with LLVM, before `argc` and `argv` are first inspected.
class InitLLVM {
 public:
  // Initializes LLVM for use by a MyLang binary. On Windows, `argc` and `argv`
...
#endif  // MYLANG_COMMON_INIT_LLVM_H_
